<div class="announcement-card position-relative">
  <div class="dropdown announcement-options">
    <button class="btn announcement-dots" type="button" data-bs-toggle="dropdown" aria-expanded="false">
      <i class="fas fa-ellipsis-v"></i>
    </button>
    <ul class="dropdown-menu shadow-sm dropdown-menu-end p-2 animate-slide-down">
      <li>
        <button type="button"
      ...
      <li class="mt-2">
        <form method="POST">
          ...
          <button type="submit" class="btn btn-warning btn-sm w-100">
            <i class="fas fa-thumbtack me-1"></i> <?= $a['is_pinned'] ? 'Unpin' : 'Pin' ?>
          </button>
        </form>
      </li>
      <li class="mt-2">
  ...
  <div class="announcement-text pe-4">
    ...
    <small class="text-muted d-block mt-2">
      <?= date("F d, Y | g:ia", strtotime($a['encoded_at'])) ?> by <?= getMemberName($conn, $a['encoded_by']) ?>
    </small>
  </div>
</div>
